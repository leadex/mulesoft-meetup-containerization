 ...
 * @row Id,contactId,The identifier of the contact in Salesforce
...
[{
	Id: vars.contactId,
	(Salutation: payload.salutation) if(!isEmpty(payload.salutation)),
	(FirstName: payload.firstName) if(!isEmpty(payload.firstName)),
	(LastName: payload.lastName) if(!isEmpty(payload.lastName)),
	(BirthDate: payload.birthDate as Date {format: "yyyy-MM-dd"}) if(!isEmpty(payload.birthDate)),
